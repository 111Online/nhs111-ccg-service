...
namespace NHS111.DataImport.CCG
{
    class Program
    {
        ...
        public static async Task LoadDOSSearchDistanceLookupdata(string xlsFilePath)
        {
            ...
            {
                _dosSearchDistancePartialLookup.Add(_regexRemoveWhitespace.Replace(partialPostcodeSheet.Cells[i, 1].Value.ToString(), string.Empty), Convert.ToInt32(partialPostcodeSheet.Cells[i, 2].Value));
            }
